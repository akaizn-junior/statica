# AGENTS.md — statica-core

Rust library: the statica build pipeline. Read [../AGENTS.md](../AGENTS.md) for project-wide context.

## Responsibility

`statica-core` transforms a site directory into static HTML. It receives [`BuildOptions`](src/build.rs) and returns a [`BuildReport`](src/build.rs). It **never reads `statica.toml`** — that is the CLI's job.
