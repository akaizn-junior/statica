...
- Content → `<slot name="field">` (filled from the bind context when present)
- Attributes → `${field}` (field must be declared in the fragment `data-bind` — no magic vars; use `data-bind="{a, b}"` or `${prop.field}`)
